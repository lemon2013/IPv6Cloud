...
<div class="content-wrapper">
		...
		<div class="box box-widget">
			...
			<div class="box-body">
				<div id="toolbar" class="toolbar">
            			<a href="javascript:;" id="btn-file-upload" class="btn btn-primary"><i class="fa fa-upload"></i> 上传</a>
					<a href="javascript:;" id="btn-file-download" class="btn btn-info"><i class="fa fa-download"></i> 下载</a>
					<a href="javascript:;" id="btn-file-del" class="btn btn-danger"><i class="fa fa-trash"></i> 删除</a>
          		</div>
	         ...
	         <form id="upload-form" class="form-horizontal" action="" encrypt="multipart/form-data" method="post">
	         	<div	 class="row">
					<div class="form-group">
						<label for="file-upload" class="col-sm-4 control-label"></label>
						<div class="col-sm-6">
	         	...
	         	<div	 class="row">
					<div class="form-group">
						<label for="file-bucket" class="col-sm-4 control-label">桶名：</label>
						<div class="col-sm-6">
							<input type="text" id="file-bucket" name="bucketName"></input>
						</div>
					</div>
	         	</div>
	         </form>
	...
	<!--share modal-->
	<div id="shareModal" class="modal fade" tabindex="-1" role="dialog" aira-labelledby="gridSystemModalLabel">
	 <div class="modal-dialog" role="document">
	   <div class="modal-content">
	     <div class="modal-header">
	       <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	       <h4 class="modal-title" id="gridSystemModalLabel">链接分享</h4>
	     </div>
	     <div class="modal-body">
	         <form id="share-form" class="form-horizontal" action="" encrypt="multipart/form-data" method="post">
	         	<div	 class="row">
					<div class="form-group">
						<label for="share-url" class="control-label"></label>
						<div class="col-sm-offset-1 col-sm-6">
							<input type="text" id="share-url" class="" style="width: 100%" name="shareUrl" value=""></input>
						</div>
						<div class="col-sm-4">
							<button id="share-btn" data-clipboard-target="#share-url" class="btn btn-default primary">复制链接</button>
						</div>
					</div>
	         	</div>
	         	<div id = "share-success-text" classname = ""></div>
	         </form>
	     </div>
	     <div class="modal-footer">
	       <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
...
<script>
	$(function() {
		$('#table').bootstrapTable({
			...
			onDblClickRow: function(row, $ele, field){
				var objStr= "bucketname=" + row.bucketName + "&" + "objname=" + row.key;
				$.ajax({
	...
	/* 文件下载代码*/
 	$("#btn-file-download").on("click", function() {
		...
		if(select.length == 0){
			...
			for(var i = 0; i < select.length; i++){
				obj.bucketname = select[0].bucketName;
				obj.objname = select[0].key;
				objStr = "bucketname=" + select[0].bucketName + "&" + "objname=" + select[0].key;
			}
			console.log(objStr);
			$.ajax({
				url: "file/downloadurl",
		...
		$("#btn-file-del").on("click", function(){
			...
			if(selects.length == 0){
				...
				for(var i = 0; i < selects.length; i++){
					dataArr.push(selects[i].bucketName + "&" + selects[i].key);
				}
				console.log(dataArr);
				$.ajax({
					url: "index/delete",
